defmodule Jikan.TrackingTest do
  ...
  describe "clients" do
    ...
    test "create_client/1 with valid data creates a client", %{user: user} do
      ...
      assert {:ok, %Client{} = client} = Tracking.create_client(user, valid_attrs)
      assert client.active == true
      assert client.name == "some name"
      assert client.contact_email == "some contact_email"
    end

    test "create_client/1 with invalid data returns error changeset", %{user: user} do
      assert {:error, %Ecto.Changeset{}} = Tracking.create_client(user, @invalid_attrs)
    end

    test "update_client/2 with valid data updates the client", %{user: user} do
      client = client_fixture(user)
    ...
    test "delete_client/1 deletes the client", %{user: user} do
      ...
      assert_raise Ecto.NoResultsError, fn -> Tracking.get_client!(user, client.id) end
    end

    test "change_client/1 returns a client changeset", %{user: user} do
      client = client_fixture(user)
  ...
  describe "projects" do
    ...
    test "create_project/1 with valid data creates a project", %{user: user, client: client} do
      valid_attrs = %{name: "some name", description: "some description", color: "#FF0000", archived: true, client_id: client.id}

      assert {:ok, %Project{} = project} = Tracking.create_project(user, valid_attrs)
    ...
    test "create_project/1 with invalid data returns error changeset", %{user: user} do
      assert {:error, %Ecto.Changeset{}} = Tracking.create_project(user, @invalid_attrs)
    end

    test "update_project/2 with valid data updates the project", %{user: user, client: client} do
      project = project_fixture(user, %{client_id: client.id})
      update_attrs = %{name: "some updated name", description: "some updated description", color: "#00FF00", archived: false}
    ...
    test "delete_project/1 deletes the project", %{user: user, client: client} do
      ...
      assert_raise Ecto.NoResultsError, fn -> Tracking.get_project!(user, project.id) end
    end

    test "change_project/1 returns a project changeset", %{user: user, client: client} do
      project = project_fixture(user, %{client_id: client.id})
  ...
  describe "time_entries" do
    ...
    test "create_time_entry/1 with invalid data returns error changeset", %{user: user} do
      assert {:error, %Ecto.Changeset{}} = Tracking.create_time_entry(user, @invalid_attrs)
    end
    ...
    test "delete_time_entry/1 deletes the time_entry", %{user: user, project: project} do
      ...
      assert_raise Ecto.NoResultsError, fn -> Tracking.get_time_entry!(user, time_entry.id) end
    end

    test "change_time_entry/1 returns a time_entry changeset", %{user: user, project: project} do
      time_entry = time_entry_fixture(user, %{project_id: project.id})
